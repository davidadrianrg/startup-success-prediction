...
class Report:
    ...
    def print_confusion_matrix(
        ...
        **kwargs,
        ...
        disp = plot_confusion_matrix(model, X_test, t_test, labels=[xlabel, ylabel])  # Show confusion matrix plot
    ...
    def print_roc_curve(
        ...
        **kwargs,
        ...
        t_test_bin = label_binarize(t_test, classes=np.arange(0, n_classes, 1))  # Recoding the labels to binary values
        ...
        for i in range(n_classes):
            ...
            ax.plot(fpr[i], tpr[i], color=colors[i], lw=1, label="ROC class %i (area = %0.3f)" % (i, roc_auc[i]))
    ...
    def print_clreport(self, t_test: np.ndarray, y_pred: np.ndarray, title:str = "Classification report"):
    ...
    def print_hpcontrast(self, *args: list, labels:list, alpha: float = 0.05):
        ...
        if pVal <= alpha:
            ...
            self.print_noformat(str_toprint + 'Hypotheses are being accepted: the models are equal')
